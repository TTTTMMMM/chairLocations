...
exports.updateDateTask = async (req, res, admin) => {
   ...
   if (
      ...
      let dateDoneEsc = escapeHTML(
         dateObj.dateAccomplished.trim().substring(0, 10)
      );
      ...
      let docID = escapeHTML(
         req.params.id.trim().substring(0, 6).toUpperCase()
      );
      const validDocIDRegex = /^[A-F0-9]{6}$/gi;
      let valid_docID = docID.match(validDocIDRegex);
      if (valid_dateDone && valid_docID) {
         admin
            .firestore()
            .collection("tasks")
            .doc(`${valid_docID[0]}`)
            ...
            .then(() => {
               ...
               console.log(
                  `Updated task completion date for [${valid_docID[0]}] to [${valid_dateDone[0]}]`
               );
               res.append("Cache-Control", "no-cache, must-revalidate");
               return res.status(200).json({
                  message: `Updated task completion date for [${valid_docID[0]}] to [${valid_dateDone[0]}]`,
               });
            })
            .catch((err) => {
               const firstLine =
         ...
         admin
            ...
            .then(() => {
               ...
               console.log(
                  `Updated task completion date for [${valid_docID[0]}] to []`
               );
               res.append("Cache-Control", "no-cache, must-revalidate");
               return res.status(200).json({
                  message: `Updated task completion date for [${valid_docID[0]}] to []`,
               });
            })
            .catch((err) => {
               const firstLine =
