...
exports.addChairLoc1 = async (req, res, admin) => {
   if (res.locals.loggedInUser.canAccess.chairLocsWrite) {
      ...
      try {
         ...
         const firstLine =
            "0994: Adding chairLoc error: " + err.message.split("\n")[0];
      ...
      try {
         ...
         const firstLine = `0992: Checking existence of ${theAssetLabel} in 'uniqueAssetLabels' collection error: ${
            err.message.split("\n")[0]
         }`;
         const errCode = err.code;
         res.status(500).render("500", { firstLine, errCode });
      ...
      try {
         ...
         const firstLine = `0972: Checking existence of ${theRentalAgent} in 'uniqueRentalAgents' collection error: ${
            err.message.split("\n")[0]
         }`;
         const errCode = err.code;
         res.status(500).render("500", { firstLine, errCode });
