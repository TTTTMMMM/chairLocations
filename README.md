...
**Big Caveat right up front:** Firebase rules only applies to operations originating from the firebase SDK, not operations originating from functions (API handlers on the server.)! You have to write access control logic code yourself as part of the handlers if you want any access control in the API.
